...
// Standard Current Time Service
#define UUID_TIME_SVC             "00001805-0000-1000-8000-00805f0b34fb"
#define UUID_CHAR_CURRENT_TIME    "00002A2B-0000-1000-8000-00805f0b34fb"
...
void ble_process(WatchState& ws) {
  ...
  if (_char_wx_code) {
    std::string v = _char_wx_code->getValue();
    if (!v.empty()) ws.weatherCode = (uint8_t)v[0];
  }
  if (_char_wx_temp) {
    std::string v = _char_wx_temp->getValue();
    if (v.size() >= 2) {
